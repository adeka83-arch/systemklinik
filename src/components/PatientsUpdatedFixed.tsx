...
import { Plus, Edit, UserPlus, Users, CalendarDays, FileText, Search, Phone, MapPin, Calendar as CalendarIcon, Trash2, RefreshCw, Printer, Filter } from 'lucide-react'
...
export default function PatientsUpdatedFixed({ accessToken }: PatientsProps) {
  ...
  const fetchSchedules = async () => {
    try {
      const response = await fetch(`${serverUrl}/control-schedules`, {
        headers: { 'Authorization': `Bearer ${accessToken}` }
      })
      const data = await response.json()
      if (data.success) {
...
<body>
  ...
  <div class="info-section">
    ...
    <div class="info-row">
      ...
      <div class="info-value">${new Date(record.visitDate).toLocaleDateString('id-ID', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric'
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      ...
                                      dengan nomor RM <strong>{patient.medicalRecordNumber}</strong>? 
          ...
          <div className="flex items-center justify-between">
            ...
            <Dialog open={scheduleDialogOpen} onOpenChange={setScheduleDialogOpen}>
              ...
              <DialogContent className="large-dialog max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  <DialogTitle className="text-xl">Tambah Jadwal Kontrol</DialogTitle>
                  <DialogDescription>
                    Buat jadwal kontrol baru untuk pasien - Cari dan pilih pasien dari tabel di bawah
                  </DialogDescription>
                </DialogHeader>
                ...
                <form onSubmit={handleScheduleSubmit} className="space-y-6">
                    ...
                    <div className="space-y-3">
                      ...
                      {scheduleSearchTerm && (
                        <div className="text-sm text-gray-600">
                          ...
                          {filteredSchedulePatients.length > 5 && ' (menampilkan 5 teratas)'}
                        </div>
                      )}
                      
                      {/* Patient Selection Table - FIXED STRUCTURE */}
                        ...
                        <Table className="w-full table-fixed">
                          <TableHeader>
                            <TableRow className="bg-blue-50">
                              ...
                              <TableHead className="text-blue-700 text-center w-[10%]">Pilih</TableHead>
                            </TableRow>
                          </TableHeader>
                          <TableBody>
                            {!scheduleSearchTerm ? (
                      ...
                      {scheduleForm.patientId && (
                        <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                          <div className="flex items-center justify-between">
                            <div>
                              <span className="text-sm font-medium text-blue-800">Pasien Terpilih:</span>
                              <div className="mt-1">
                                {(() => {
                                  const selectedPatient = patients.find(p => p.id === scheduleForm.patientId)
                                  return selectedPatient ? (
                                    <div className="text-sm">
                                      ...
                                      <Badge variant="outline" className="font-mono text-xs">
                                        {selectedPatient.medicalRecordNumber || 'Belum ada RM'}
                                      </Badge>
                                      <span className="mx-2 text-gray-500">•</span>
                                      <span className="text-gray-600">{selectedPatient.phone}</span>
                                    </div>
                                  ) : (
                                    <span className="text-sm text-gray-500">Pasien tidak ditemukan</span>
                                  )
                                })()}
                              </div>
                            </div>
                            <Button
                              ...
                              size="sm"
                              variant="ghost"
                              onClick={() => setScheduleForm(prev => ({ ...prev, patientId: '' }))}
                  ...
                  <div className="flex justify-end space-x-4 pt-6 border-t border-gray-200 mt-8">
                    ...
                    >
                      {loading ? 'Menyimpan...' : 'Simpan Jadwal Kontrol'}
                    </Button>
                  </div>
                </form>
              </DialogContent>
                      ...
                      schedules.map((schedule) => {
                        ...
                        return (
                          <TableRow key={schedule.id}>
                            ...
                            <TableCell>
                              {new Date(schedule.controlDate).toLocaleDateString('id-ID')}
                            </TableCell>
              ...
              <Dialog open={recordDialogOpen} onOpenChange={setRecordDialogOpen}>
                ...
                <DialogContent className="large-dialog max-h-[90vh] overflow-y-auto">
                  ...
                  <form onSubmit={handleRecordSubmit} className="space-y-6">
                      ...
                      <div className="space-y-4">
                          ...
                          <Table className="w-full table-fixed">
                            <TableHeader>
                              <TableRow className="bg-green-50">
                                ...
                                <TableHead className="text-green-700 text-center w-[10%]">Pilih</TableHead>
                              </TableRow>
                            </TableHeader>
                            <TableBody>
                              {!recordSearchTerm ? (
                                ...
                                filteredRecordPatients.slice(0, 10).map((patient) => (
                                  <TableRow
                                    key={patient.id}
                                    className={`cursor-pointer hover:bg-gray-50 ${
                        ...
                        {recordForm.patientId && (
                          <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                            <div className="flex items-center justify-between">
                              <div>
                                <span className="text-sm font-medium text-green-800">Pasien Terpilih:</span>
                                <div className="mt-1">
                                  {(() => {
                                    const selectedPatient = patients.find(p => p.id === recordForm.patientId)
                                    return selectedPatient ? (
                              ...
                              <Button
                                ...
                                size="sm"
                                variant="ghost"
                                onClick={() => setRecordForm(prev => ({ ...prev, patientId: '' }))}
                    ...
                    {/* Display selected patient info in edit mode */}
                    {editRecordMode && recordForm.patientId && (
                      <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                        <div className="flex items-center gap-2">
                          <Users className="h-5 w-5 text-green-600" />
                          <span className="text-sm font-medium text-green-800">Pasien:</span>
                          {(() => {
                            const selectedPatient = patients.find(p => p.id === recordForm.patientId)
                            return selectedPatient ? (
                              <div className="flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                      ...
                      <div className="space-y-4">
                        <Label htmlFor="complaint" className="text-base font-semibold">Keluhan</Label>
                        <Textarea
                          id="complaint"
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                      <div className="space-y-4">
                        <Label htmlFor="prescription" className="text-base font-semibold">Resep Obat</Label>
                        <Textarea
                          id="prescription"
                          value={recordForm.prescription}
                          onChange={(e) => setRecordForm(prev => ({ ...prev, prescription: e.target.value }))}
                          placeholder="Resep obat (opsional)"
                          className="resize-none text-base"
                          rows={3}
                        />
                      </div>
                          ...
                          <TableRow key={record.id}>
                            ...
                            <TableCell>
                              <div className="max-w-xs">
                                <span className="truncate" title={record.complaint}>
                                  {record.complaint}
                                </span>
                              </div>
                            </TableCell>
                            <TableCell>
                              <div className="max-w-xs">
                                <span className="truncate" title={record.diagnosis}>
                                  {record.diagnosis}
                                </span>
                              </div>
                            </TableCell>
                                ...
                                <AlertDialog>
                                  ...
                                  <AlertDialogContent>
                                    <AlertDialogHeader>
                                      ...
                                      <AlertDialogDescription>
                                        Apakah Anda yakin ingin menghapus rekam medis pasien <strong>{record.patientName}</strong> 
                                        tanggal <strong>{new Date(record.visitDate).toLocaleDateString('id-ID')}</strong>? 
