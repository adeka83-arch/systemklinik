...
export function ControlScheduleNotification({ accessToken, onScheduleUpdate }: ControlScheduleNotificationProps) {
  ...
  const markAsCompleted = async (scheduleId: string) => {
    try {
      ...
      // Find the schedule to get complete data
      ...
      // Send all current schedule data plus the new status
      const updateData = {
        ...schedule,
        status: 'completed',
...
Mohon konfirmasi kehadiran Anda dengan membalas pesan ini atau hubungi kami di:
📞 WhatsApp: 085283228355

Jika ada perubahan jadwal, silakan hubungi kami sebelumnya.
                  ...
                  <div className="flex flex-col gap-2 ml-4">
                    ...
                    {patient && (
                      <Button 
                        variant="outline" 
                        size="sm"
                        onClick={() => handleWhatsAppReminder(patient, schedule)}
                        className="border-green-200 text-green-700 hover:bg-green-50"
                        title={`Kirim reminder WhatsApp ke ${patient.phone}`}
                      >
