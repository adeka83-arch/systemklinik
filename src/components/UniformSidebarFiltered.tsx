...
// MENU ITEMS - AKAN DIFILTER BERDASARKAN LEVEL AKSES
const ALL_MENU_ITEMS: MenuItem[] = [
  ...
  { id: 'system-configuration', label: 'Konfigurasi Sistem', icon: Cog },
  { id: 'backup-manager', label: 'Backup Database', icon: Database }
]
...
export function UniformSidebar({ activeTab, onTabChange, onLogout, clinicSettings, isOpen, onToggle, isMobile }: UniformSidebarProps) {
  ...
  // Filter menu items berdasarkan level akses
  const visibleMenuItems = ALL_MENU_ITEMS.filter((item) => {
    ...
    console.log(`🎯 Menu Filter: "${item.label}" (${item.id}) - Required: ${getLevelName(requiredLevel)}, Has Access: ${hasPageAccess}`)
    return hasPageAccess
  })

  // Debug informasi menu yang terfilter
  console.log(`📊 Filtered Menu Summary: ${visibleMenuItems.length}/${ALL_MENU_ITEMS.length} items visible for level ${getLevelName(currentLevel)}`)
  console.log(`🔍 Visible items:`, visibleMenuItems.map(item => item.label))
  ...
  // Force refresh when level changes
  useEffect(() => {
    console.log(`🔄 Level changed to: ${getLevelName(currentLevel)} (${currentLevel})`)
    console.log(`📋 Current page access config:`, getPageAccess)
    
    // Force re-render of menu items
    const newVisibleItems = ALL_MENU_ITEMS.filter((item) => {
      const requiredLevel = getPageAccess(item.id)
      return hasAccess(requiredLevel)
    })
    console.log(`🎯 Re-filtered menu items: ${newVisibleItems.length} items visible`)
  }, [currentLevel, hasAccess, getPageAccess, getLevelName])
    ...
    <div className={`hidden lg:flex bg-white border-r border-pink-200 transition-all duration-300 ${
      ...
      <nav className="flex-1 p-2 space-y-1 overflow-y-auto">
        {!isCollapsed ? (
          ...
          // Collapsed view - just icons (filtered)
          <div className="space-y-1">
            {visibleMenuItems.map((item, index) => {
              const Icon = item.icon
              const isActive = activeTab === item.id
                ...
                >
                  <Button
                    ...
                    className={`w-full justify-center p-2 transition-all duration-300 ${
                      isActive 
                        ? 'bg-gradient-to-r from-pink-100 to-purple-100 text-pink-800 border-l-4 border-pink-600 shadow-md transform scale-105' 
