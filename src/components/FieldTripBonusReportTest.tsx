...
interface FieldTripBonusReportTestProps {
  accessToken: string
...
export function FieldTripBonusReportTest({ accessToken, onPrint, fieldTripSalesData = [] }: FieldTripBonusReportTestProps) {
  ...
  const processFieldTripData = (sales: any[]) => {
    ...
    sales.forEach(sale => {
      ...
      if (sale.selectedDoctors && Array.isArray(sale.selectedDoctors)) {
        sale.selectedDoctors.forEach((doctor: any) => {
          const key = `doctor-${doctor.doctorName || doctor.name || 'Unknown Doctor'}`
      ...
      if (sale.selectedEmployees && Array.isArray(sale.selectedEmployees)) {
        sale.selectedEmployees.forEach((employee: any) => {
          const key = `employee-${employee.employeeName || employee.name || 'Unknown Employee'}`
          const name = employee.employeeName || employee.name || 'Unknown Employee'
    ...
    console.log('Processed field trip records:', recordsArray)
        ...
        sale.selectedDoctors.forEach((doctor: any) => {
          ...
          if (recordsMap.has(key)) {
            ...
            recordsMap.set(key, {
              ...
              fieldTripCount: newCount,
              averageAmount: newCount > 0 ? newAmount / newCount : 0,
              dates: [...(existing.dates || []), sale.date]
            })
          } else {
            const fee = doctor.fee || 0
            recordsMap.set(key, {
