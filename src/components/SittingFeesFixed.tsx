...
import { Plus, Edit, Trash2, Clock, ArrowDown, Settings, User, PlusCircle, AlertCircle, RefreshCw } from 'lucide-react'
import { toast } from 'sonner@2.0.3'
import { serverUrl } from '../utils/supabase/client'
import { processValidDoctors } from '../utils/doctorNameCleaner'
import { ServerHealthChecker } from './ServerHealthChecker'
import { SittingFeesDiagnostic } from './SittingFeesDiagnostic'
...
export function SittingFees({ accessToken }: SittingFeesProps) {
  ...
  const fetchSittingFees = async () => {
    try {
      console.log('🏥 Fetching sitting fees...')
      console.log('🔗 Using URL:', `${serverUrl}/sitting-fees`)
      ...
      if (data.success) {
        ...
        console.log('✅ Sitting fees loaded:', sortedSittingFees.length)
      } else {
  ...
  const fetchDoctors = async () => {
    try {
      ...
      console.log('🔗 Using URL:', `${serverUrl}/doctors`)
      ...
      if (!response.ok) {
        const errorText = await response.text()
        console.error('❌ Doctors server error response:', errorText)
        throw new Error(`HTTP ${response.status}: ${response.statusText}`)
      }
  ...
  const fetchSittingFeeSettings = async () => {
    try {
      console.log('⚙️ Fetching sitting fee settings...')
      console.log('🔗 Using URL:', `${serverUrl}/doctor-sitting-fee-settings`)
      ...
      if (data.success) {
        ...
        console.log('✅ Sitting fee settings loaded:', data.settings?.length || 0)
      } else {
  ...
  const handleEditSittingFee = (sittingFee: SittingFee) => {
    ...
    setEditFormData({
      ...
      description: '' // Set empty for now, could be enhanced later
    })
    setEditDialogOpen(true)
  }

  // Handle delete sitting fee
  ...
  if (error) {
    return (
      <div className="space-y-6">
        ...
        {/* API Diagnostic */}
        <SittingFeesDiagnostic accessToken={accessToken} />
      </div>
    )
  }
